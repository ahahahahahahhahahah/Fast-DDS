        ...
        uint8_t ident)
{
    ...
    while (nullptr != p_aux0)
    {
        /*
            <xs:complexType name="flowControllerDescriptorType">
                <xs:all>
                    ...
                    <xs:element name="adaptive_feedback_slow_windows" type="uint32" minOccurs="0" maxOccurs="1"/>
        ...
        for (p_aux1 = p_aux0->FirstChildElement(); p_aux1 != NULL; p_aux1 = p_aux1->NextSiblingElement())
        {
            ...
            else if (strcmp(name, ADAPTIVE_FEEDBACK_SLOW_WINDOWS) == 0)
            {
                if (XMLP_ret::XML_OK != getXMLUint(p_aux1,
                        &flow_controller_descriptor->adaptive_feedback_slow_windows, ident))
                {
                    return XMLP_ret::XML_ERROR;
                }
            }
