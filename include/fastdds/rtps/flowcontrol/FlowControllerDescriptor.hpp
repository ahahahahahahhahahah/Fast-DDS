...
struct FlowControllerDescriptor
{
    ...
    //! Consecutive slow-feedback windows required before decreasing budget.
    uint32_t adaptive_feedback_slow_windows = 2;

    //! Minimum current-budget utilization in percent before positive feedback can increase the budget.
